...
export default function Home() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setUserUploadedImg(URL.createObjectURL(file))
      setMode('upload')
    }
  }
      ...
      {tryOnOpen && tryOnProduct && (
        <div className="fixed inset-0 z-[100] bg-[#080808] flex flex-col items-center p-6 overflow-y-auto">
          ...
          <div className="flex gap-3 mb-8 bg-zinc-950 p-2 rounded-2xl border border-white/5">
            <button onClick={() => setMode('camera')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase ${mode === 'camera' ? 'bg-white text-black' : 'text-zinc-600'}`}>Live Camera</button>
            <label className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase cursor-pointer ${mode === 'upload' ? 'bg-white text-black' : 'text-zinc-600'}`}>
              Upload Photo
              <input type="file" className="hidden" accept="image/*" onChange={handleFileUpload} />
            </label>
          </div>

          <div className="w-full max-w-md aspect-[3/4] rounded-[56px] overflow-hidden bg-black border border-white/10 relative shadow-2xl">
            {mode === 'camera' ? (
              ...
              <div className="relative w-full h-full bg-zinc-900">
                {userUploadedImg ? (
                  <>
                    <img src={userUploadedImg} className="w-full h-full object-cover" />
                    <img src={tryOnImage} className="absolute inset-0 w-full h-full object-contain p-16 drop-shadow-2xl" />
                  </>
                ) : (
                  <div className="flex items-center justify-center h-full text-zinc-700 text-[10px] font-black uppercase">Select a photo from gallery</div>
                )}
              </div>
            )}
          </div>
          ...
          {tryOnProduct.variants && (
            <div className="mt-8 flex gap-3">
              {tryOnProduct.variants.map(v => (
                <button key={v.colorName} onClick={() => setTryOnImage(v.tryOnImage)} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase border-2 transition-all ${tryOnImage === v.tryOnImage ? 'border-cyan-400 text-cyan-400 bg-cyan-400/5' : 'border-white/10 text-zinc-700'}`}>{v.colorName}</button>
              ))}
            </div>
          )}
          ...
          <button onClick={() => window.open(`${CONTACT_LINKS.whatsapp}&text=Order: ${tryOnProduct.name} Size: ${tryOnSize}`)} className="mt-10 w-full max-w-md bg-white text-black py-6 rounded-full font-black uppercase text-xs shadow-2xl hover:bg-cyan-400 transition-all">Confirm Order via WhatsApp</button>
        </div>
      )}
